...
pub fn tensor_to_png_data_url_rgb(img: &Tensor) -> Result<String> {
	...
	for y in 0..h { for x in 0..w {
		let i = (y*w + x)*3;
		buf[i] = (v[0][y][x].clamp(0.0,1.0)*255.0).round() as u8;
...
pub fn show_tensor_gray_captioned(
	...
	let img_html = match tensor_to_png_data_url_gray(img) {
		Ok(url) => format!(
			"<div><img style=\"max-width:100%; image-rendering: pixelated; border: 1px solid rgba(0,0,0,0.12);\" src=\"{url}\"/></div>"
		),
		Err(_) => String::new(),
	};
