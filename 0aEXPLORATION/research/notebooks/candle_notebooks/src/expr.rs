...
impl Parser {
    ...
    fn eat(&mut self, kind: TokKind) -> Result<()> {
    ...
    fn expr(&mut self) -> Result<Ast> {
        ...
        while matches!(self.cur().text.as_str(), "+" | "-") {
            let op = self.cur().text.chars().next().unwrap();
            self.pos += 1;
            let rhs = self.term()?;
            node = Ast::Binary(Box::new(node), op, Box::new(rhs));
        }
        Ok(node)
    }
    ...
    fn power(&mut self) -> Result<Ast> {
        ...
        if self.cur().text == "^" {
            self.pos += 1;
            let rhs = self.power()?;
            node = Ast::Binary(Box::new(node), '^', Box::new(rhs));
        }
        Ok(node)
    }
    ...
    fn primary(&mut self) -> Result<Ast> {
        match self.cur().kind {
            TokKind::Num => {
                let v = self.cur().text.clone();
                self.pos += 1;
                Ok(Ast::Num(v.parse::<f64>()?))
            }
            TokKind::Ident => {
...
impl ExprEnv {
    pub fn new(x: Tensor, y: Tensor, params: HashMap<String, f64>) -> Result<Self> {
        Ok(Self { x, y, params })
    }
}

fn scalar(env: &ExprEnv, v: f64) -> Result<Tensor> {
    Ok(Tensor::new(&[v], env.x.device())?
        .to_dtype(DType::F64)?
        .broadcast_as(env.x.shape())?)
}
    ...
    fn eval(node: &Ast, env: &ExprEnv) -> Result<Tensor> {
        match node {
            ...
            Ast::Var(name) => match name.as_str() {
                ...
                other => {
                    if let Some(v) = env.params.get(other) {
                        scalar(env, *v)
                    } else {
                        Err(anyhow!("unknown identifier: {other}"))
                    }
                }
            },
            Ast::Unary(op, inner) => {
                let t = eval(inner, env)?;
                match op {
                    '+' => Ok(t),
                    '-' => Ok((-1f64 * t)?),
                    _ => unreachable!(),
            ...
            Ast::Call(name, args) => {
                ...
                let t = match lower.as_str() {
                    "sin" | "cos" | "tanh" | "exp" | "log" | "sqrt" | "abs" | "floor" | "ceil" => {
                        if eval_args.len() != 1 {
                            return Err(anyhow!("{lower} expects 1 arg"));
                        }
                        let a = &eval_args[0];
                        match lower.as_str() {
                            "sin" => a.sin()?,
                    ...
                    "pow" | "min" | "max" => {
                        if eval_args.len() != 2 {
                            return Err(anyhow!("{lower} expects 2 args"));
                        }
                        let a = &eval_args[0];
                        let b = &eval_args[1];
                        match lower.as_str() {
                    ...
                    "clamp" => {
                        if eval_args.len() != 3 {
                            return Err(anyhow!("clamp expects 3 args"));
                        }
                        eval_args[0]
                            .maximum(&eval_args[1])?
                            .minimum(&eval_args[2])?
