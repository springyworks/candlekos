...
impl StreamedTensorDisplay {
    ...
    fn create_initial_tensor_b(device: &Device) -> Result<Tensor> {
        ...
        for y in 0..TENSOR_HEIGHT {
            for x in 0..TENSOR_WIDTH {
                ...
                let value =
                    (nx * std::f32::consts::PI).sin() * (ny * std::f32::consts::PI).cos() * 0.5
                        + 0.5;
                data.push(value);
            }
        }
    ...
    fn update_tensors_gpu_stream(&mut self) -> Result<()> {
        ...
        if self.tensor_a.dims() != [TENSOR_HEIGHT, TENSOR_WIDTH] {
            println!(
                "⚠️  Tensor A shape mismatch: {:?}, reinitializing...",
                self.tensor_a.dims()
            );
        ...
        if self.tensor_b.dims() != [TENSOR_HEIGHT, TENSOR_WIDTH] {
            println!(
                "⚠️  Tensor B shape mismatch: {:?}, reinitializing...",
                self.tensor_b.dims()
            );
        ...
        match self.mode {
            ...
            FeedbackMode::Direct => {
                ...
                let transformed_a = self.apply_gpu_rotation(
                    &self.tensor_a,
                    self.time,
                    1.0 + self.feedback_strength * 0.1,
                )?;
            ...
            FeedbackMode::Cross => {
                ...
                self.tensor_a = (self.tensor_a.affine(0.9, 0.0)?
                    + feedback_a.affine(self.feedback_strength as f64, 0.0)?)?;
                self.tensor_b = (self.tensor_b.affine(0.9, 0.0)?
                    + feedback_b.affine(self.feedback_strength as f64, 0.0)?)?;
            }
            ...
            FeedbackMode::Interference => {
                ...
                self.tensor_a = (self.tensor_a.affine(0.95, 0.0)?
                    + phase_shift.affine((self.feedback_strength * 0.3) as f64, 0.0)?)?;
                self.tensor_b = (self.tensor_b.affine(0.95, 0.0)?
                    + phase_shift.affine((self.feedback_strength * 0.7) as f64, 0.0)?)?;
            }
            ...
            FeedbackMode::Convolution => {
                ...
                self.tensor_b = (self.tensor_b.affine(0.9, 0.0)?
                    + conv_b.affine(self.feedback_strength as f64, 0.0)?)?;
            }
            ...
            FeedbackMode::FFTAnalysis => {
                ...
                self.tensor_a =
                    (processed_mag.affine(0.7, 0.0)? + processed_phase.affine(0.3, 0.0)?)?;
                self.tensor_b = (self.tensor_b.affine(0.95, 0.0)?
                    + processed_mag.affine(self.feedback_strength as f64, 0.0)?)?;
            }
            ...
            FeedbackMode::MatrixOps => {
                ...
                let eigenish =
                    diagonal_sum.to_scalar::<f32>()? / (TENSOR_WIDTH * TENSOR_HEIGHT) as f32;
                let scaled_product =
                    mat_product.affine(self.feedback_strength as f64, eigenish as f64)?;

                self.tensor_a =
                    (self.tensor_a.affine(0.85, 0.0)? + scaled_product.affine(0.15, 0.0)?)?;
                // Fix: Use element-wise operations to maintain [128, 128] shape
                // instead of matrix multiplication which could cause shape issues
                let feedback_transform =
                    self.tensor_a.affine(self.feedback_strength as f64, 0.0)?;
                self.tensor_b =
                    (self.tensor_b.affine(0.9, 0.0)? + feedback_transform.affine(0.1, 0.0)?)?;
            }
            ...
            FeedbackMode::Nonlinear => {
                ...
                let nonlinear_mix = (tanh_a.affine(0.4, 0.0)?
                    + relu_a.affine(0.3, 0.0)?
                    + sigmoid_b.affine(0.3, 0.0)?)?;

                self.tensor_a = (self.tensor_a.affine(0.8, 0.0)?
                    + nonlinear_mix.affine(self.feedback_strength as f64, 0.0)?)?;
                self.tensor_b = (self.tensor_b.affine(0.9, 0.0)?
                    + tanh_a.affine(self.feedback_strength as f64 * 0.5, 0.0)?)?;
            }
            ...
            FeedbackMode::Reduction => {
                ...
                self.tensor_a = (self.tensor_a.affine(0.7, 0.0)?
                    + mean_broadcast.affine(self.feedback_strength as f64, 0.0)?)?;
                self.tensor_b = (self.tensor_b.affine(0.8, 0.0)?
                    + sum_broadcast.affine(self.feedback_strength as f64 * 0.1, 0.0)?)?;
            }
            ...
            FeedbackMode::Spectral => {
                ...
                let spectral_feedback =
                    self.apply_gpu_rotation(&phase_mod, self.time * 0.8, 0.98)?;

                self.tensor_a = (self.tensor_a.affine(0.85, 0.0)?
                    + spectral_feedback.affine(self.feedback_strength as f64, 0.0)?)?;
                self.tensor_b = (self.tensor_b.affine(0.9, 0.0)?
                    + filtered.affine(self.feedback_strength as f64 * 0.6, 0.0)?)?;
            }
        }
    ...
    fn create_feedback_kernel(&self) -> Result<Tensor> {
        ...
        let kernel_data = [-0.1, -0.2, -0.1, 0.0, 1.0, 0.0, 0.1, 0.2, 0.1];
    ...
    fn handle_input(&mut self) {
        ...
        if self.window.is_key_down(Key::Key4) {
            self.mode = FeedbackMode::StreamTest;
        }

        // Advanced modes [5-0]
        if self.window.is_key_down(Key::Key5) {
            self.mode = FeedbackMode::Convolution;
        }
        if self.window.is_key_down(Key::Key6) {
            self.mode = FeedbackMode::FFTAnalysis;
        }
        if self.window.is_key_down(Key::Key7) {
            self.mode = FeedbackMode::MatrixOps;
        }
        if self.window.is_key_down(Key::Key8) {
            self.mode = FeedbackMode::Nonlinear;
        }
        if self.window.is_key_down(Key::Key9) {
            self.mode = FeedbackMode::Reduction;
        }
        if self.window.is_key_down(Key::Key0) {
            self.mode = FeedbackMode::Spectral;
        }
        ...
        if self.window.is_key_down(Key::T) {
            self.mode = FeedbackMode::MatrixOps;
        }
    ...
    fn run(&mut self) -> Result<()> {
        ...
        while self.window.is_open() && !self.window.is_key_down(Key::Escape) {
            ...
            // Update window
            self.window
                .update_with_buffer(&self.display_buffer, WINDOW_WIDTH, WINDOW_HEIGHT)
                .unwrap();
...
fn monitor_tensor(tensor: &Tensor) -> Result<String> {
    ...
    Ok(format!(
        "min={:.3}, max={:.3}, mean={:.3}, shape={:?}, device={:?}",
        min_val,
        max_val,
        mean_val,
        tensor.shape(),
