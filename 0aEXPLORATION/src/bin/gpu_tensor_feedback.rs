...
impl GpuTensorFeedback {
    ...
    fn monitor_tensor(&self, name: &str, tensor: &Tensor) -> Result<()> {
        ...
        println!(
            "📊 {} | Shape: {:?} | Device: {:?} | Min: {:.3}, Max: {:.3}, Mean: {:.3}",
            name,
            tensor.dims(),
            tensor.device(),
            min_val,
    ...
    fn apply_rotation(&self, input: &Tensor) -> Result<Tensor> {
        ...
        let _y_coords: Vec<f32> = (0..TENSOR_HEIGHT)
            .map(|y| y as f32 - TENSOR_HEIGHT as f32 / 2.0)
            .collect();
        let _x_coords: Vec<f32> = (0..TENSOR_WIDTH)
            .map(|x| x as f32 - TENSOR_WIDTH as f32 / 2.0)
            .collect();
                ...
                if tensor_x < TENSOR_WIDTH && tensor_y < TENSOR_HEIGHT {
                    ...
                    let color = match self.mode {
                        FeedbackMode::Direct => {
                            ...
                            (0xFF000000u32)
                                | ((intensity / 4) as u32) << 16
                                | ((intensity / 2) as u32) << 8
                                | (intensity as u32)
                        }
                        FeedbackMode::Cross => {
