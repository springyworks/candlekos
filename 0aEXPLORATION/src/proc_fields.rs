...
pub fn meshgrid_2d(h: usize, w: usize, device: &Device) -> Result<(Tensor, Tensor)> {
    let xs: Vec<f32> = (0..w)
        .map(|i| (i as f32 / (w - 1).max(1) as f32) * 2.0 - 1.0)
        .collect();
    let ys: Vec<f32> = (0..h)
        .map(|i| (i as f32 / (h - 1).max(1) as f32) * 2.0 - 1.0)
        .collect();
...
pub fn expr_field(h: usize, w: usize, t: f32, expr: &str, device: &Device) -> Result<Tensor> {
    ...
    let parsed: Expr = expr
        .parse()
        .map_err(|e| candle_core::Error::Msg(format!("expr parse error: {e}")))?;
...
where
    F: Fn(f32) -> (u8, u8, u8),
{
    let Ok(v) = t.flatten_all().and_then(|f| f.to_vec1::<f32>()) else {
        return vec![];
    };
    let dims = t.dims();
    let (h, w) = match dims {
