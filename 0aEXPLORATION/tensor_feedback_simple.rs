...
impl SimpleTensorFeedback {
    ...
    fn monitor_tensor(&self, name: &str, tensor: &Tensor) -> Result<()> {
        ...
        println!("📊 {} | Shape: {:?} | Min: {:.3}, Max: {:.3}, Mean: {:.3}", 
                 name, tensor.dims(), min_val, max_val, mean_val);
        Ok(())
    }
    ...
    fn apply_rotation(&self, input: &Tensor) -> Result<Tensor> {
        ...
        for y in 0..TENSOR_HEIGHT {
            for x in 0..TENSOR_WIDTH {
                ...
                if new_x >= 0.0 && new_x < TENSOR_WIDTH as f32 && 
                   new_y >= 0.0 && new_y < TENSOR_HEIGHT as f32 {
    ...
    fn apply_wave_transform(&self, input: &Tensor) -> Result<Tensor> {
        ...
        let result_data: Vec<f32> = data_a.iter().zip(data_b.iter())
            .map(|(&a, &b)| a * b)
            .collect();
    ...
    fn update_feedback(&mut self) -> Result<()> {
        match self.mode {
            ...
            FeedbackMode::Interference => {
                ...
                let interference_data: Vec<f32> = data_a.iter().zip(data_b.iter())
                    .map(|(&a, &b)| a * b)
                    .collect();
                let interference = Tensor::from_vec(interference_data, self.tensor_a.shape(), &self.device)?;
