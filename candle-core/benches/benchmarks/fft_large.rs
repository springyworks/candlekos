...
use super::{BenchDeviceHandler, BenchDevice};

fn bench_2d(c: &mut Criterion) {
    if std::env::var("CANDLE_FFT_LARGE").ok().map(|v| v=="1" || v.eq_ignore_ascii_case("true")).unwrap_or(false) == false { return; }
...
fn fft_large_benches(c: &mut Criterion) { bench_2d(c); }
