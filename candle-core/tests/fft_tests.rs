...
fn test_cpu_fft_basic() -> Result<()> {
    ...
    assert_eq!(
        ...
        "Peak should be at frequency bin {}",
        freq
    );
...
fn test_cpu_fftn_high_dimensional() -> Result<()> {
    ...
    let data: Vec<f32> = (0..total_size).map(|i| (i as f32 * 0.01).sin()).collect();
