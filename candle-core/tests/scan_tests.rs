use candle_core::{D, Device, Result, Tensor, test_device, test_utils::to_vec2_round};
...
fn scan_2d_rowwise(device: &Device) -> Result<()> {
    ...
    assert_eq!(
        ...
        &[[0.0, 1.0, 3.0, 6.0], [0.0, 5.0, 11.0, 18.0]]
    );
...
fn scan_3d_tensor(device: &Device) -> Result<()> {
    ...
    assert_eq!(
        result_vec,
        &[[[1.0, 2.0], [4.0, 6.0]], [[5.0, 6.0], [12.0, 14.0]]]
    );
...
test_device!(
    scan_1d_edge_cases,
    scan_1d_edge_cases_cpu,
    scan_1d_edge_cases_cuda,
    scan_1d_edge_cases_metal
);
test_device!(
    scan_2d_rowwise,
    scan_2d_rowwise_cpu,
    scan_2d_rowwise_cuda,
    scan_2d_rowwise_metal
);
test_device!(
    scan_2d_colwise,
    scan_2d_colwise_cpu,
    scan_2d_colwise_cuda,
    scan_2d_colwise_metal
);
test_device!(
    scan_3d_tensor,
    scan_3d_tensor_cpu,
    scan_3d_tensor_cuda,
    scan_3d_tensor_metal
);
test_device!(
    scan_different_dtypes,
    scan_different_dtypes_cpu,
    scan_different_dtypes_cuda,
    scan_different_dtypes_metal
);
test_device!(
    scan_strided_tensor,
    scan_strided_tensor_cpu,
    scan_strided_tensor_cuda,
    scan_strided_tensor_metal
);
test_device!(
    scan_performance_large,
    scan_performance_large_cpu,
    scan_performance_large_cuda,
    scan_performance_large_metal
);
test_device!(
    scan_numerical_stability,
    scan_numerical_stability_cpu,
    scan_numerical_stability_cuda,
    scan_numerical_stability_metal
);
test_device!(
    scan_memory_patterns,
    scan_memory_patterns_cpu,
    scan_memory_patterns_cuda,
    scan_memory_patterns_metal
);
test_device!(
    scan_api_wrappers,
    scan_api_wrappers_cpu,
    scan_api_wrappers_cuda,
    scan_api_wrappers_metal
);
...
mod cuda_specific {
    ...
    fn scan_extreme_sizes_cuda() -> Result<()> {
        ...
        for size in sizes {
            let input: Vec<f32> = (0..size)
                .map(|x| if x % 1000 == 0 { 1.0 } else { 0.0 })
                .collect();
            let tensor = Tensor::new(input.as_slice(), &device)?;
            let result = tensor.cumsum(D::Minus1)?;
            let result_vec = result.to_vec1::<f32>()?;
            ...
            let expected_final = if size == 0 {
                0.0
            } else {
                (1 + (size - 1) / 1000) as f32
            };
    ...
    fn scan_multi_gpu_consistency_cuda() -> Result<()> {
        ...
        if false {
            // Placeholder - would need proper device enumeration
            ...
            assert_eq!(
                vec0, vec1,
                "Results should be identical across CUDA devices"
            );
        }
