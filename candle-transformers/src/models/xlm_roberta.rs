...
impl XLMRobertaSelfAttention {
    ...
    fn forward(
        ...
        let (key_layer, value_layer, attention_mask) = match (is_cross_attention, past_key_value) {
            (true, Some((pk, pv))) => {
                let attention_mask = encoder_attention_mask
                    .expect(
                        "encoder_attention_mask required when cross-attention with past_key_value",
                    )
                    .clone();
                (pk.clone(), pv.clone(), Some(attention_mask))
            }
            (true, None) => {
                let enc = encoder_hidden_states
                    .expect("encoder_hidden_states required for cross-attention");
                ...
                let attention_mask = encoder_attention_mask
                    .expect("encoder_attention_mask required for cross-attention");
            ...
            (false, Some((pk, pv))) => {
                ...
                let mut value_layer =
                    self.transpose_for_scores(&self.value.forward(hidden_states)?)?;
